...
const findSegmentData = async (query, aggregation, companyName, startDate, endDate) => {
  ...
  let result = await elasticSearch(query, aggregation,index, startDate, endDate);
  ...
  let genderData = {};
  for await (const genderObj of result.gender.buckets) {
   genderData[genderObj.key] = genderObj.amount.value;
  }
  return {
    gender: genderData,
    phone_number: result.phone_number.value,
